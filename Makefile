...
pg_build_image:
	...
	$(DOCKER_COMPOSE) build $(DOCKER_COMMON)
	$(DOCKER_COMPOSE) build pg
	$(DOCKER_COMPOSE) build pg_build_docker_prefix
...
pg_integration_test: clean_compose
	...
	@if echo "$(TEST)" | grep -Fqe "pgbackrest"; then\
		$(DOCKER_COMPOSE) build pg_pgbackrest;\
	fi
	@if echo "$(TEST)" | grep -Fqe "pg_ssh_"; then\
		$(DOCKER_COMPOSE) build ssh;\
	fi

	$(DOCKER_COMPOSE) up --exit-code-from $(TEST) $(TEST)
	# Run tests with dependencies if we run all tests
	@if [ "$(TEST)" = "pg_tests" ]; then\
		$(DOCKER_COMPOSE) build pg_pgbackrest ssh swift pg_wal_perftest_with_throttling &&\
		$(DOCKER_COMPOSE) up --exit-code-from pg_ssh_backup_test pg_ssh_backup_test &&\
		$(DOCKER_COMPOSE) up --exit-code-from pg_storage_swift_test pg_storage_swift_test &&\
		$(DOCKER_COMPOSE) up --exit-code-from pg_storage_ssh_test pg_storage_ssh_test &&\
		$(DOCKER_COMPOSE) up --exit-code-from pg_pgbackrest_backup_fetch_test pg_pgbackrest_backup_fetch_test &&\
		$(DOCKER_COMPOSE) down &&\
		sleep 5 &&\
		$(DOCKER_COMPOSE) up --exit-code-from pg_wal_perftest_with_throttling pg_wal_perftest_with_throttling ;\
	fi
	make clean_compose
...
pg_int_tests_only:
	$(DOCKER_COMPOSE) build pg_tests
	$(DOCKER_COMPOSE) up --exit-code-from pg_tests pg_tests
...
mysql_integration_test: deps mysql_build unlink_brotli load_docker_common
	./link_brotli.sh
	$(DOCKER_COMPOSE) build mysql $(MYSQL_TEST)
	$(DOCKER_COMPOSE) up --force-recreate --exit-code-from $(MYSQL_TEST) $(MYSQL_TEST)
...
mariadb_integration_test: unlink_brotli load_docker_common
	./link_brotli.sh
	$(DOCKER_COMPOSE) build mariadb mariadb_tests
	$(DOCKER_COMPOSE) up --force-recreate --exit-code-from mariadb_tests mariadb_tests
...
fdb_integration_test: load_docker_common
	...
	$(DOCKER_COMPOSE) build fdb_tests
	$(DOCKER_COMPOSE) up --force-recreate --renew-anon-volumes --exit-code-from fdb_tests fdb_tests
...
redis_integration_test: load_docker_common
	$(DOCKER_COMPOSE) build redis redis_tests
	$(DOCKER_COMPOSE) up --exit-code-from redis_tests redis_tests
...
etcd_integration_test: load_docker_common
	$(DOCKER_COMPOSE) build etcd etcd_tests
	$(DOCKER_COMPOSE) up --exit-code-from etcd_tests etcd_tests
...
gp_integration_test: load_docker_common
	$(DOCKER_COMPOSE) build gp gp_tests
	$(DOCKER_COMPOSE) up --exit-code-from gp_tests gp_tests

st_test: deps pg_build unlink_brotli st_integration_test

st_integration_test: load_docker_common
	$(DOCKER_COMPOSE) build st_tests
	$(DOCKER_COMPOSE) up --exit-code-from st_tests st_tests
